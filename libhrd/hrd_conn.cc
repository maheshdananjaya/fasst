...
struct hrd_ctrl_blk* hrd_ctrl_blk_init(int local_hid, int port_index,
	...
	volatile void *prealloc_dgram_buf, int num_dgram_qps,
	int dgram_buf_size, int dgram_buf_shm_key)
{
#if HRD_CONNECT_IB_ATOMICS == 1
	hrd_red_printf("HRD: Connect-IB atomics enabled. This QP setup has not "
		"been tested for non-atomics performance.\n");
	sleep(1);
#endif

	hrd_red_printf("HRD: creating control block %d: port %d, socket %d, "
		"conn qps %d, UC %d, conn buf %d bytes (key %d), "
		"dgram qps %d, dgram buf %.3f MB (key %d)\n",
		local_hid, port_index, numa_node_id,
		num_conn_qps, use_uc, conn_buf_size, conn_buf_shm_key,
		num_dgram_qps, (double) dgram_buf_size / M_1, dgram_buf_shm_key);
	...
	if(num_conn_qps == 0 && num_dgram_qps == 0) {
		hrd_red_printf("HRD: Control block initialization without QPs. Are you"
	 ...
	 */
	if(num_conn_qps >= 1) {
		cb->conn_qp = (struct ibv_qp **)
			malloc(num_conn_qps * sizeof(struct ibv_qp *));
		cb->conn_cq = (struct ibv_cq **)
			malloc(num_conn_qps * sizeof(struct ibv_cq *));
...
{
	int i;
	assert(cb->conn_qp != NULL &&
		cb->conn_cq != NULL && cb->pd != NULL && cb->ctx != NULL);
	assert(cb->num_conn_qps >= 1 && cb->dev_port_id >= 1);

	for(i = 0; i < cb->num_conn_qps; i++) {
		cb->conn_cq[i] = ibv_create_cq(cb->ctx, HRD_SQ_DEPTH, NULL, NULL, 0);
...
void hrd_connect_qp(struct hrd_ctrl_blk *cb,
	int n, struct hrd_qp_attr *remote_qp_attr)
{
...
void hrd_publish_dgram_qp(struct hrd_ctrl_blk *cb, int n, const char *qp_name)
{
	...
	for(i = 0; i < len; i++) {
		...
		//assert(IB_PHYS_PORT != cb->dev_port_id); //Clemson fails
